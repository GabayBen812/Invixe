      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {lessons.map((lesson, idx) => (
          <View key={lesson.id} style={styles.nodeRow}>
            <View style={[styles.node, lesson.unlocked ? styles.nodeUnlocked : styles.nodeLocked]}>
              <Text style={styles.nodeText}>{lesson.title}</Text>
              {lesson.unlocked ? (
                <TouchableOpacity style={styles.startBtn} onPress={() => navigation.navigate('Lesson', { lessonId: lesson.id })}>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 26, fontWeight: 'bold', color: 'white', marginBottom: 32, textShadowColor: '#222', textShadowOffset: { width: 2, height: 2 }, textShadowRadius: 6 },
  nodeRow: { alignItems: 'center' },
  node: { width: 300, backgroundColor: '#fff', borderRadius: 32, padding: 24, alignItems: 'center', marginBottom: 16, elevation: 4 },
  nodeUnlocked: { borderColor: '#4caf50', borderWidth: 2 },
  nodeLocked: { borderColor: '#aaa', borderWidth: 2, opacity: 0.6 },
  nodeText: { fontSize: 20, fontWeight: 'bold', color: '#1e355e', marginBottom: 8 },
  startBtn: { backgroundColor: '#4caf50', borderRadius: 20, paddingVertical: 8, paddingHorizontal: 32 },
  startBtnText: { color: 'white', fontSize: 18, fontWeight: 'bold' },
  lockedText: { fontSize: 22, color: '#aaa', marginTop: 4 },
  connector: { width: 4, height: 32, backgroundColor: '#4caf50', marginBottom: 8 },
}); 