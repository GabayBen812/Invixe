...
export const UserProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchUserData = async (email?: string) => {
    try {
      ...
      if (!userEmail) {
        ...
        const res = await fetch('http://10.0.0.16:4000/api/user/progress');
        if (!res.ok) {
      ...
      const res = await fetch(`http://10.0.0.16:4000/api/user/progress?email=${encodeURIComponent(userEmail)}`);
      if (!res.ok) {
        throw new Error(`Failed to fetch user data: ${res.status}`);
      }
      const data = await res.json();
  ...
  const setLessonAttempts = async (attempts: LessonAttempt[]) => {
    try {
      ...
      const userEmail = currentUserEmail;
      const res = await fetch('http://10.0.0.16:4000/api/user/lesson-attempts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          email: userEmail,
  ...
  const setCoins = async (coins: number) => {
    setCoinsState(coins);
    const userEmail = currentUserEmail;
    await fetch('http://10.0.0.16:4000/api/user/currency', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
