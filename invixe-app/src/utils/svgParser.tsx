...
function parseSVGElements(svgCode: string): React.ReactElement[] {
  ...
  for (const match of lineMatches) {
    ...
    elements.push(
      <Line
        ...
        strokeWidth={lineProps["stroke-width"] || lineProps.strokeWidth}
      />,
    );
  }
  ...
  for (const match of imageMatches) {
    ...
    const href =
      imageProps.href ||
      imageProps["xlink:href"] ||
      imageProps["xmlns:xlink"] ||
      "";
    if (href) {
      elements.push(
        <Image
          ...
          href={href}
          preserveAspectRatio={
            imageProps.preserveAspectRatio || "xMidYMid meet"
          }
        />,
      );
    }
  ...
  for (const match of useMatches) {
    ...
    if (href && href.startsWith("#")) {
      ...
      console.log(
        "SVG parser: <use> element found referencing:",
        id,
        "- not fully supported",
      );
    }
...
function parseAttributes(elementString: string): Record<string, string> {
  ...
  if (attributes.style) {
    const styleParts = attributes.style.split(";");
