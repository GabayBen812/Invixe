...
export default function HtmlText({
  ...
  if (!isHtml) {
    return <Text style={[defaultTextStyle, style]}>{value}</Text>;
  }
  ...
  if (looksLikeMalformedHtml(value)) {
    ...
    return <Text style={[defaultTextStyle, style]}>{plain}</Text>;
  }

  const mergedTagsStyles = { ...defaultTagsStyles, ...customTagsStyles };
