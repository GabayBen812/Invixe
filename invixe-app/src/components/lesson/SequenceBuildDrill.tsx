...
export default function SequenceBuildDrill({ slotsCount, options, correctSequence, correctSequences, submitText = 'אישור', correctExplanation, wrongExplanation, onSubmit, onSubmitTriggerRef, onStateChange }: Props) {
  ...
  const optionInitialPositions = useRef<Record<string, { x: number; y: number }>>({});
  const optionRefs = useRef<Record<string, View | null>>({});
  const optionLayouts = useRef<Record<string, { x: number; y: number; width: number; height: number }>>({});
  const optionOriginalBankLayouts = useRef<Record<string, { x: number; y: number; width: number; height: number }>>({});
  ...
  const detectDropZone = (screenX: number, screenY: number): { zone: 'slot' | 'bank' | 'middle'; slotIndex?: number; slotCenter?: { x: number; y: number } } => {
  ...
  const attachPanResponder = (option: SequenceOption) => {
    ...
    return PanResponder.create({
      onStartShouldSetPanResponder: () => {
        ...
        return draggingOptionIdRef.current === null || draggingOptionIdRef.current === option.id;
      },
      ...
      onPanResponderGrant: (evt) => {
        ...
        if (draggingOptionIdRef.current !== null && draggingOptionIdRef.current !== option.id) {
          return;
        }
        ...
        if (optionRef) {
          (optionRef as any).measureInWindow((x: number, y: number, width: number, height: number) => {
      ...
      onPanResponderRelease: (_, gesture) => {
        ...
        if (optionsRowRef.current) {
          (optionsRowRef.current as any).measureInWindow((x: number, y: number, width: number, height: number) => {
        ...
        if (optionRef) {
          (optionRef as any).measureInWindow((x: number, y: number, width: number, height: number) => {
            ...
            requestAnimationFrame(() => {
              ...
              if (dropZone.zone === 'slot' && dropZone.slotIndex !== undefined && dropZone.slotCenter && currentCenter) {
          ...
          if (originalCenter) {
            const dropZone = detectDropZone(screenX, screenY);
            if (dropZone.zone === 'slot' && dropZone.slotIndex !== undefined && dropZone.slotCenter) {
              handleSlotReplacement(dropZone.slotIndex, option.id);
  ...
  }, [onSubmitTriggerRef, showingExplanation, placed, isCorrect, correctExplanation, wrongExplanation]);
        ...
        {Array.from({ length: slotsCount }).map((_, i) => {
          ...
          return (
            <View 
              ...
              style={[
                styles.slotCircle,
                submitted && isSlotFilled && (isSlotCorrect ? styles.slotCorrect : styles.slotWrong)
              ]}
            />
          );
        })}
              ...
              {isDragging && (
                <View
                  style={[
                    styles.optionShadow,
                    {
                      width: optionOriginalBankLayouts.current[o.id]?.width || 40,
                      height: optionOriginalBankLayouts.current[o.id]?.height || 120,
                    },
                  ]}
                />
              )}
